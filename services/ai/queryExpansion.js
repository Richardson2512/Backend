 ...
 * Generates focus areas using Groq (Llama 3.1 70B) with Ollama fallback
 */
class QueryExpansionService {
  ...
  static async generateQueryExpansion(query) {
    try {
      logger.info(`🤖 Generating AI query expansion for: "${query}" using Groq (Llama 3.1 70B) with Ollama fallback`);

      // Optimized prompt for Llama 3.1 70B
...
Generate JSON for "${query}":`;
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          logger.info(`🔄 Attempt ${attempt}/3: Calling AI service for query expansion`);
          
          const aiResponse = await AIClient.call(prompt, {
            temperature: 0.7, // Balanced temperature for creativity and accuracy
            max_tokens: 2500, // Increased for better JSON generation
          });
