...
/**
 * Segregation Service
 * Categorizes posts by sentiment/intent using Groq (Llama 3.1 70B) with Ollama fallback
 */
class SegregationService {
  ...
  static async performCategorySpecificAnalysis(posts, expandedQuery, category) {
    try {
      logger.info(`🔍 Performing ${category} analysis for: "${expandedQuery}" using Groq (Llama 3.1 70B) with Ollama fallback`);

      const semanticContext = await AIUtils.generateSemanticContext(expandedQuery, AIClient);
...
Return maximum 15 most relevant posts for ${category}.`;
  ...
  static async aiSentimentAnalysis(posts, query) {
    try {
      ...
      logger.info(`🤖 AI analyzing sentiment of ${maxPosts} posts using Groq (Llama 3.1 70B) with Ollama fallback...`);
      ...
      const semanticContext = await AIUtils.generateSemanticContext(query, AIClient);
