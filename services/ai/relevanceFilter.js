 ...
 * Filters posts for relevance using Groq (Llama 3.1 70B) with Ollama fallback
 */
class RelevanceFilterService {
  ...
  static async filterRelevantPosts(posts, query) {
    try {
      logger.info(`🔍 AI filtering ${posts.length} posts for relevance to: "${query}" using Groq (Llama 3.1 70B) with Ollama fallback`);
  ...
  static async filterBatchRelevance(posts, query, offset) {
    try {
      ...
      const [ratingInsights, semanticContext] = await Promise.all([
        ...
        AIUtils.generateSemanticContext(query, AIClient)
      ]);
