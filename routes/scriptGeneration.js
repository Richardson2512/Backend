...
class ScriptGenerationService {
  static async generateScript(request) {
    try {
      logger.info(`🎬 Generating ${request.contentType} script for ${request.category} category using Groq (Llama 3.1 70B) with Ollama fallback`);
...
router.get('/health', (req, res) => {
  ...
  res.json({
    ...
    aiProviders: status,
    primaryModel: 'llama-3.1-70b-versatile (Groq)',
    backupModel: 'llama3.1:8b (Ollama)'
  });
});
